...
async function snapshot() {
  return evaluate(`(() => ({
    ...
    semanticFrame: document.querySelector('[data-testid="avatar"]')?.dataset.frame,
    rendererPresentation: document.querySelector('.point-rig-renderer')?.dataset.currentOutfit,
    rendererFrame: document.querySelector('.point-rig-renderer')?.dataset.currentFrame,
...
await submit("боньк2");
await waitFor(
  `document.querySelector('[data-testid="avatar"]')?.dataset.presentation === 'base-debug'
    && document.querySelector('[data-testid="avatar"]')?.dataset.frame === 'private-playful-alt'
    && document.querySelector('.point-rig-renderer')?.dataset.currentOutfit === 'base-debug'
    && document.querySelector('.point-rig-renderer')?.dataset.currentFrame === 'private-playful-alt'`,
  3_000,
  "legacy bonk2 mannequin pose",
);
await new Promise((resolve) => setTimeout(resolve, 500));
const legacyBonk = await snapshot();
const legacyScreenshot = await command("Page.captureScreenshot", { format: "png", captureBeyondViewport: false });
await writeFile(new URL("bonk2-legacy.png", outputDir), Buffer.from(legacyScreenshot.data, "base64"));

await waitFor(
  `document.querySelector('[data-testid="avatar"]')?.dataset.presentation === 'gym'
    && document.querySelector('[data-testid="avatar"]')?.dataset.frame === 'neutral'`,
  5_000,
  "bonk2 return to previous outfit",
);
await new Promise((resolve) => setTimeout(resolve, 700));
const legacyRestored = await snapshot();
...
await waitFor(
  `document.querySelector('[data-testid="avatar"]')?.dataset.presentation === 'base-debug'
    && document.querySelector('[data-testid="avatar"]')?.dataset.frame === 'private-playful'
    && document.querySelector('.point-rig-renderer')?.dataset.currentFrame === 'private-playful'`,
  3_000,
  "corrected base bonk pose",
);
await new Promise((resolve) => setTimeout(resolve, 500));
const correctedBonk = await snapshot();
const correctedScreenshot = await command("Page.captureScreenshot", { format: "png", captureBeyondViewport: false });
await writeFile(new URL("bonk-corrected.png", outputDir), Buffer.from(correctedScreenshot.data, "base64"));

await waitFor(
  `document.querySelector('[data-testid="avatar"]')?.dataset.presentation === 'base-debug'
    && document.querySelector('[data-testid="avatar"]')?.dataset.frame === 'neutral'`,
  5_000,
  "corrected bonk return to base idle",
);
...
const valid = base.presentation === "base-debug"
  ...
  && legacyBonk.presentation === "base-debug"
  && legacyBonk.semanticFrame === "private-playful-alt"
  && legacyBonk.rendererFrame === "private-playful-alt"
  && legacyBonk.storedOutfit === "gym"
  && legacyRestored.presentation === "gym"
  && legacyRestored.storedOutfit === "gym"
  && correctedBonk.presentation === "base-debug"
  && correctedBonk.semanticFrame === "private-playful"
  && correctedBonk.rendererFrame === "private-playful"
...
const result = { valid, legacyBonk, legacyRestored, base, correctedBonk, restored, summer };
